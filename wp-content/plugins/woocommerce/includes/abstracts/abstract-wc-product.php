...
class WC_Product extends WC_Abstract_Legacy_Product {
	 ...
	 * @return string
	 */
	public function get_model_url( $context = 'view' ) {
		global $wpdb;
		$model_url = $wpdb->get_var($wpdb->prepare(
			"SELECT model_url FROM {$wpdb->prefix}wc_product_meta_lookup WHERE product_id = %d",
			$this->get_id()
		));
		return $model_url;
	}
	...
	*/
	 ...
	 *
	 * @since 3.0.0
	 * @param string $model_url URL to 3D model.
	 */
	public function set_model_url( $model_url ) {
		global $wpdb;
		$this->set_prop( 'model_url', $model_url );
		// Update directly in the lookup table as well
		if ( $this->get_id() ) {
			$wpdb->update(
				$wpdb->prefix . 'wc_product_meta_lookup',
				array( 'model_url' => $model_url ),
				array( 'product_id' => $this->get_id() )
			);
		}
	}
